	...
	methods: {
		...
		btnPitch(item) {
			if (item.pitchType) {
				...
				if (Number(this.howMany) > 5) {
					this.showMessage('文件最多传出5个');
					return;
				}
				this.howMany = this.howMany + 1;
			}
		...
		async queryBaiduGetDetails() {
			...
			try {
				...
				if(Number(listArr.length)===0){
					this.showMessage('请选择文档')
					return
				}
				const result = await baiduGetDetails({
					fid: listArr.join(',')
				});
				let results = this.order_no;
				...
				for (let [index, vals] of result.list.entries()) {
					...
					if (results.order_no) {
						params.order_no = results.order_no;
					}
					params.reset_file_name = `百度文档${index + 1}.pdf`;
					//上传文件 生成订单
					this.showUpLoading = true;
					results = await printFileUpload(result, params);
				}
				this.order_no = results;
				//跳转到订单页面
				uni.navigateTo({
					url: `/pages/index/filePrint/order/order?orderType=2&orderNo=${
						results.order_no
					}`
				});
			} catch (e) {
