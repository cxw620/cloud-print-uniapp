<template>
	<view>
		<view class="printing">
			<!-- 照片打印  列表-->
			<copyOrder :detail="orderDetail" :spec="spec" />
			<!-- 照片打印  信息-->
			<orderInformation :info="orderDetail" />
			<view class="btns">
				<view class="btn-class-black">提交订单</view>
...
export default {
	...
	onLoad(option) {
		...
		// this.getOrderDetail('OD201911272839678976');
		...
		uni.$on('topreview', e => {
			...
			let imgList = [];
			for (let i in e) {
				console.log(e[i].file_path);
				imgList.push(e[i].file_path);
			}

			uni.navigateTo({
				url:
					'/pages/index/photoPrint/preview/preview?imgList=' +
