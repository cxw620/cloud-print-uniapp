	...
	<view class="imgage-upload-contc">
		...
		<view class="upimg" v-else>
			...
			<image
				...
				@click="addimg"
			/>
			<text class="photograph-icon-text" @click="addimg">拍摄文档</text>
		</view>
		<!-- 编辑裁剪上传的图片 -->
		<image-cropper
			:src="tempFilePathScr"
...
export default {
	...
	methods: {
		...
		async uploadFile(data) {
			...
			let params = { upload_type: 1, scene: 4 };
			try {
				let result = '';
				for (let [index, el] of data.entries()) {
					...
					params.reset_file_name = `文件复印${index + 1}${el.imgUrl.substring(
