<template>
	<view>
		<view class="printing">
			<!-- 照片打印  列表-->
			<copyOrder :detail="orderDetail" :spec="spec"  v-if="orderDetail"/>
			<!-- 照片打印  信息-->
			<orderInformation :info="orderDetail"  v-if="orderDetail"/>
			<view class="btns">
...
import copyOrder from '@/components/copyOrder/copyOrder.vue';
...
export default {
	...
	onLoad(option) {
		...
		// this.getOrderDetail('OD2019112735665088');
		...
		uni.$on('topreview',(e)=>{
			...
			let imgList=[];
			for(let i in e){
				imgList.push(e[i].file_path)
			};
			uni.navigateTo({
				url:'/pages/index/photoPrint/preview/preview?imgList='+JSON.stringify(imgList)
			})
