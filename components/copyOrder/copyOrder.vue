	...
	<view>
		<!-- 文档打印  列表-->
				...
				<view class="each">
						...
						<view class="eachs">
							<view class="eachs-top-copy">
								<view class="eachs-top-copy-left">
									...
									<view class="eachs-top-img" v-if="order.goods_list">
										<image
											:src="item.file_path"
											mode=""
								...
								<view class="eachs-top-copy-right" @click="toPreview(order.goods_list)">
									<text>打印预览</text>
					...
					<!-- 			<view
						class="btn-examine"
						@click="order.showMenu = !order.showMenu"
						v-if="documentType === 1"
					>
						<view class="iconfont icontop" v-if="order.showMenu"></view>
						<view class="iconfont iconxiaoyuhao-copy" v-else></view>
					</view> -->
				</view>
...
export default {
	...
	watch: {
		...
		'order.goods_page_start'(val) {
			...
			if (
				val !== 1 &&
				this.order.goods_list[0].goods_page_end !== this.order.goods_list[0].all_goods_page &&
	...
	methods: {
		 ...
		 */
		 toPreview(list) {
			...
			uni.navigateTo({
				url:
					'/pages/index/photoPrint/preview/preview?imgList='+JSON.stringify(imgList)
			});
		},
		...
		changeGoodSku(id, index) {
			...
			uni.$emit('resetPrice', {
				...
				params: { goods_id: this.order.goods_list[0].goods_id, sku_id: skuObj.price_sku_id }
			});
		},
