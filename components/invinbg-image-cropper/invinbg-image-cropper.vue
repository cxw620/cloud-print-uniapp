...
export default {
	...
	methods: {
		...
		init() {
			this.rotate = 0;
			this.scale = 1;
			this.cropW = uni.upx2px(this.cropWidth);
			this.cropH =  uni.upx2px(this.cropHeight);
			console.log(this.compileImgindex)
			// uni.showLoading({
			// 	title: '图片加载中...'
			// });
				...
				uni.canvasToTempFilePath(
					{
						...
						success: res => {
							...
							event.detail.tempFilePath = res.tempFilePath;
								event.detail.src = this.src;
								console.log(this.compileImgindex)
								event.detail.compileImgindex=this.compileImgindex
							_this.show = false;
